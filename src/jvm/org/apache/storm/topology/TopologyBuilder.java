...
import static org.apache.storm.spout.ReplicationSpout.REPLICATION_COMPONENT_ID;
 ...
 *
 * builder.setSpout("1", new TestWordSpout(true), 5); builder.setSpout("2", new
 * TestWordSpout(true), 3); builder.setBolt("3", new TestWordCounter(), 3)
 * .fieldsGrouping("1", new Fields("word")) .fieldsGrouping("2", new
 * Fields("word")); builder.setBolt("4", new TestGlobalCount())
 * .globalGrouping("1");
 *
 * Map conf = new HashMap(); conf.put(Config.TOPOLOGY_WORKERS, 4);
 * 
 ...
 * Utils.sleep(10000); cluster.shutdown(); ```
 *
...
public class TopologyBuilder {
	...
	private boolean hasReplicationBolt = false;
	...
	public StormTopology createTopology() {
		if (hasStatefulBolt && hasReplicationBolt) {
			throw new IllegalStateException("Cannot apply replication along with the existing checkpointing method.");
		}

		Map<String, Bolt> boltSpecs = new HashMap<>();
		Map<String, SpoutSpec> spoutSpecs = new HashMap<>();
		maybeAddCheckpointSpout();
		maybeAddReplicationSpout();

		for (String boltId : _bolts.keySet()) {
			IRichBolt bolt = _bolts.get(boltId);
			bolt = maybeAddCheckpointTupleForwarder(bolt);
			bolt = maybeAddReplicationTupleForwarder(bolt);
			ComponentCommon common = getComponentCommon(boltId, bolt);
			try {
				maybeAddReplicationInputs(common);
				maybeAddCheckpointInputs(common);
				boltSpecs.put(boltId, new Bolt(ComponentObject.serialized_java(Utils.javaSerialize(bolt)), common));
			} catch (RuntimeException wrapperCause) {
				if (wrapperCause.getCause() != null
						&& NotSerializableException.class.equals(wrapperCause.getCause().getClass())) {
					throw new IllegalStateException("Bolt '" + boltId + "' contains a non-serializable field of type "
							...
							+ wrapperCause.getCause().getMessage() + " "
							+ "should be instantiated within the prepare method of '" + boltId + " at the earliest.",
							wrapperCause);
				}
				throw wrapperCause;
			}
		...
		for (String spoutId : _spouts.keySet()) {
			...
			try {
				spoutSpecs.put(spoutId,
						new SpoutSpec(ComponentObject.serialized_java(Utils.javaSerialize(spout)), common));
			} catch (RuntimeException wrapperCause) {
				if (wrapperCause.getCause() != null
						&& NotSerializableException.class.equals(wrapperCause.getCause().getClass())) {
					throw new IllegalStateException("Spout '" + spoutId + "' contains a non-serializable field of type "
							+ wrapperCause.getCause().getMessage() + ", "
							+ "which was instantiated prior to topology creation. "
							+ wrapperCause.getCause().getMessage() + " "
							+ "should be instantiated within the prepare method of '" + spoutId + " at the earliest.",
							wrapperCause);
				}
				throw wrapperCause;
			}
	...
	private void maybeAddReplicationInputs(ComponentCommon common) {
		if(hasReplicationBolt){
			addReplicationInputs(common);
		}
	}

	private void addReplicationInputs(ComponentCommon common) {
		 Set<GlobalStreamId> replicationInputs = new HashSet<>();
	        for (GlobalStreamId inputStream : common.get_inputs().keySet()) {
	            String sourceId = inputStream.get_componentId();
	            if (_spouts.containsKey(sourceId)) {
	            	replicationInputs.add(new GlobalStreamId(REPLICATION_COMPONENT_ID, REPLICATION_STREAM_ID));
	            } else {
	            	replicationInputs.add(new GlobalStreamId(sourceId, REPLICATION_STREAM_ID));
	            }
	        }
	        for (GlobalStreamId streamId : replicationInputs) {
	        	common.put_to_inputs(streamId, Grouping.all(new NullStruct()));
	        }
	}

	private IRichBolt maybeAddReplicationTupleForwarder(IRichBolt bolt) {
		if (hasReplicationBolt && !(bolt instanceof ReplicationStatefulBoltExecutor)) {
			bolt = new ReplicationTupleForwarder(bolt);
		}
		return bolt;
	}

	private void maybeAddReplicationSpout() {
		if (hasReplicationBolt) {
			setSpout(REPLICATION_COMPONENT_ID, new ReplicationSpout(), 1);
		}
	}
	 ...
	 *             if {@code parallelism_hint} is not positive
	 */
	 ...
	 * intended for windowing operations. The
	 * {@link IWindowedBolt#execute(TupleWindow)} method is triggered for each
	 ...
	 * requires its state (of computation) to be saved. When this bolt is
	 * initialized, the {@link IStatefulBolt#initState(State)} method is invoked
	 * after
	 * {@link IStatefulBolt#prepare(Map, TopologyContext, OutputCollector)} but
	 * before {@link IStatefulBolt#execute(Tuple)} with its previously saved
	 * state.
	 * <p>
	 * The framework provides at-least once guarantee for the state updates.
	 * Bolts (both stateful and non-stateful) in a stateful topology are
	 * expected to anchor the tuples while emitting and ack the input tuples
	 * once its processed.
	 * </p>
	 * 
	 * @param id
	...
	public <T extends RepState> BoltDeclarer setBolt(String id, IRepStatefulBolt<T> bolt) throws IllegalArgumentException{
		return setBolt(id,bolt,null);
	}
	 ...
	 *             if {@code parallelism_hint} is not positive
	 */
	public <T extends State> BoltDeclarer setBolt(String id, IStatefulBolt<T> bolt, Number parallelism_hint)
			throws IllegalArgumentException {
		hasStatefulBolt = true;
		return setBolt(id, new StatefulBoltExecutor<T>(bolt), parallelism_hint);
	}
	
	public <T extends RepState> BoltDeclarer setBolt (String id, IRepStatefulBolt<T> bolt, Number parallelism_hint)
			throws IllegalArgumentException{
		hasReplicationBolt = true;
		return setBolt(id, new ReplicationStatefulBoltExecutor<T>(bolt),parallelism_hint);
	}

	/**
	 * Define a new bolt in this topology. This defines a stateful windowed
	 * bolt, intended for stateful windowing operations. The
	 * {@link IStatefulWindowedBolt#execute(TupleWindow)} method is triggered
	 * for each window interval with the list of current events in the window.
	 * During initialization of this bolt
	 * {@link IStatefulWindowedBolt#initState(State)} is invoked with its
	 ...
	 *            components that want to consume this bolt's outputs.
	 * @param bolt
	 *            the stateful windowed bolt
	 * @param <T>
	 *            the type of the state (e.g.
	 *            {@link org.apache.storm.state.KeyValueState})
	 * @return use the returned object to declare the inputs to this component
	 * @throws IllegalArgumentException
	 *             if {@code parallelism_hint} is not positive
	 */
	public <T extends State> BoltDeclarer setBolt(String id, IStatefulWindowedBolt<T> bolt)
			throws IllegalArgumentException {
		return setBolt(id, bolt, null);
	}
	 ...
	 *             if {@code parallelism_hint} is not positive
	 */
	public <T extends State> BoltDeclarer setBolt(String id, IStatefulWindowedBolt<T> bolt, Number parallelism_hint)
			throws IllegalArgumentException {
		hasStatefulBolt = true;
	...
	/**
	 * If the topology has at least one stateful bolt add a
	 * {@link CheckpointSpout} component to the topology.
	 */
	...
	/**
	 * For bolts that has incoming streams from spouts (the root bolts), add
	 * checkpoint stream from checkpoint spout to its input. For other bolts,
